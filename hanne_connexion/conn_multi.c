...
int next_id_to_assign = 0; // Variable globale pour gérer les IDs fixes 

//  La fonction pour ajouter un contact
void add_peer_if_new(struct sockaddr_in new_peer_addr) {
    // 1. On vérifie si le joueur existe déjà
    for (int i = 0; i < nb_joueur_connecte; i++) {
        if (paire_connected[i].addr.sin_addr.s_addr == new_peer_addr.sin_addr.s_addr &&
            ...
            // Mise à jour du temps (pour éviter le timeout)
            paire_connected[i].dernier_vu = time(NULL); 
            return; 
        }
    }

    // 2. Si c'est un nouveau et qu'il y a de la place
    if (nb_joueur_connecte < NB_JOUEUR_MAX) {
        ...
        printf("[ALERTE] Serveur plein, impossible d'ajouter un nouveau joueur.\n");
    }
}
...
//deconnexion du joueur dans le cas d'une inactivité prolongée
// Cette fonction vérifie les timeouts et permet à ton binôme de réagir
//timeout_sec est le temps d'inactivité après lequel on considère que le joueur est parti
int check_and_get_inactive_paire(int timeout_sec) {
    time_t now = time(NULL);
    for (int i = 0; i < nb_joueur_connecte; i++) {
        if (difftime(now, paire_connected[i].dernier_vu) > timeout_sec) {
            return remove_peer(i); 
     // On retourne l'ID à tanou pour qu'il fasse son broadcast
        }
    }
