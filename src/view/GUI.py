...
class GUI(View):
    ...
    def draw_minimap(self):
        ...
        for unit in self.battlefield.troupes.values():
            color =DARK
            # Determine if the unit is an ally or enemy based on ownership and diplomacy
            color = RED if self._is_ally(unit) else BLUE  # minimap
    ...
    def draw_info_panel(self):
        ...
        for i, g in enumerate(self.generaux[:4]):
            ...
            # ── Incoherence counter – top-center, drawn last (always on top) ──
        nb = self.battlefield.nb_pb_incoherence_handled
        font_big = pygame.font.SysFont(None, 46)
        text_str = str(nb)

        # Shadow
        shadow = font_big.render(text_str, True, (30, 30, 30))
        shadow_x = (self.screen_w - shadow.get_width()) // 2
        self.screen.blit(shadow, (shadow_x + 2, margin + 2))

        # Value
        surf = font_big.render(text_str, True, (255, 215, 50))
        surf_x = (self.screen_w - surf.get_width()) // 2
        self.screen.blit(surf, (surf_x, margin))
    ...
    def draw_camera_view(self):
        ...
        for unit in self.battlefield.troupes.values():
            ...
            if (ux >= self.zoom_x and ux < self.zoom_x + capture_w and
                ...
                id = 1 if self._is_ally(unit) else 2          # camera view
                ...
                if key in self.unit_images:
                    ...
                    if getattr(unit, 'network_locked', False):
                        # On dessine un petit cercle blanc "sur la tête" (au centre horizontal, en haut)
                        # CELL_SIZE // 2 pour centrer, et un petit décalage de 5 pixels vers le haut
                        indicator_pos = (int(rel_x + CELL_SIZE // 2), int(rel_y))
                        pygame.draw.circle(camera_surface, (255, 255, 255), indicator_pos, 4)
                        # Optionnel : un contour noir pour qu'il soit visible même sur fond clair
                        pygame.draw.circle(camera_surface, (0, 0, 0), indicator_pos, 4, 1)
