...
// Sauvegarde du dernier handshake reçu de Python (Logique Awa stable)
...
// Fonction pour demander la liste des autres joueurs à un pair (Logique Awa stable)
void envoyer_demande_pairs(SOCKET_T reseau_fd, struct sockaddr_in addr_pair) {
    char json_demande[] = "{\"type\":\"demande_pairs\"}";
    EnteteUDP env;
    memset(&env, 0, sizeof(env));
    env.taille_payload = htons(strlen(json_demande));
    env.type_message = 3; // Type 3 = demande de liste
    env.id_expediteur = 0;
    env.num_sequence = 0;
    ...
    if (buf != NULL) {
        ...
        sendto(reseau_fd, buf, taille, 0, (struct sockaddr*)&addr_pair, sizeof(addr_pair));
        free(buf);
        printf("[CONNECT] Demande de la liste des pairs envoyée.\n");
    }
}
...
int main(int argc, char *argv[]) {
   ...
   SOCKET_T reseau_fd = initialiser_ma_connexion(); 
    ...
    if (reponse[0] == 'o' || reponse[0] == 'O') {
        ...
        // On demande les autres joueurs à ce premier contact
    ...
    while(1){
        ...
        int max_fd = (int)(sock > reseau_fd ? sock : reseau_fd) + 1;
                ...
                if (json != NULL) {
                    ...
                    if (cJSON_IsString(type_item) && strcmp(type_item->valuestring, "connected") == 0) {
                        ...
                        if (cJSON_IsNumber(id_item)) {
                            ...
                            printf("[SYSTÈME] Mon ID : %d\n", id_item->valueint);
                        }
                    } else {
                        // Sauvegarde du handshake pour les futurs joueurs (Relais)
        ...
        if (FD_ISSET(reseau_fd, &fds)) {
            ...
            if(json_propre != NULL){
                ...
                if (msg != NULL) {
                    ...
                    // Gestion automatique de la liste des pairs (Relais)
                    if (type_item != NULL && strcmp(type_item->valuestring, "liste_pairs") == 0) {
                        ...
                        if (pairs_array != NULL) {
                            ...
                            for (int i = 0; i < nb; i++) {
                                ...
                                cJSON *port = cJSON_GetObjectItem(pair, "port");
                                if (ip && port) {
                                    struct sockaddr_in n_addr;
                                    memset(&n_addr, 0, sizeof(n_addr));
                                    n_addr.sin_family = AF_INET;
                                    n_addr.sin_addr.s_addr = inet_addr(ip->valuestring);
                                    n_addr.sin_port = htons(port->valueint);
                                    add_peer_if_new(n_addr);
                                }
                            }
                        }
                    } else if (type_item != NULL && strcmp(type_item->valuestring, "nouveau_pair") == 0) {
                        // Un nouveau joueur arrive, on lui envoie notre handshake sauvegardé
                        if (strlen(dernier_handshake) > 0) {
                            diffusion_message_sens1(dernier_handshake, reseau_fd, 5);
                        }
                    } else {
                        // Message normal pour le jeu
